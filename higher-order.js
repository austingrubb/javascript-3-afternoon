...
//Code Here
let evenNumbers = mixedNumbers.filter(i => i % 2 === 0)
...
//Code Here
let totalPopulation = populations.reduce((t,i) => t + i)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(value => value.CP > 200)
...
let ordersTotal = orders.map(value => value.price * (value.tax +1))
...
// let bobsTotal = purchases.filter(v => v.owner === "Bob").map(v => v.price).reduce((total, v) => total + v);